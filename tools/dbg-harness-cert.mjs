...
/* FIXED harness: the derivative index is the COORDINATE index (0=t, 1=r, 2=th, 3=phi), and the
   metric depends only on r and theta, so dg[1] = d/dr and dg[2] = d/dtheta. */
function deriv(r, th) {
...
/* ---- 1. SELF-CERTIFICATION against the analytic Schwarzschild values ---- */
const rS = 8, thS = 1.2;
const libS = christoffel(rS, thS, 1, 0, 0);
const { G: GS } = gamma(rS, thS);
const checks = [
  ['Gamma^r_tt', libS[1][0][0], GS[1][0][0], (1*(rS-2)/(rS*rS*rS))],
  ['Gamma^r_thth', libS[1][2][2], GS[1][2][2], -(rS-2)],
  ['Gamma^th_thth', libS[2][2][2], GS[2][2][2], 0],
  ['Gamma^t_tr', libS[0][2][2], GS[0][2][2], 0],
];
console.log('SELF-CERTIFICATION of the harness (a = 0, Schwarzschild):');
let ok = true;
for (const [name, lib, mine, exact] of checks) {
  const pass = Math.abs(mine - lib) <= 1e-6 * Math.max(Math.abs(lib), 1);
  if (!pass) ok = false;
  console.log('  ' + name.padEnd(16) + ' analytic ' + exact.toExponential(4).padStart(13) + '   library ' + lib.toExponential(4).padStart(13) + '   mine ' + mine.toExponential(4).padStart(13) + '   ' + (pass ? 'PASS' : 'FAIL'));
}
console.log('  harness certified: ' + ok);
console.log('');
if (!ok) { console.log('ABORT: the harness is still wrong; no further conclusions.'); process.exit(0); }
/* ---- 2. now the two checks that matter ---- */
function compat(r, th) {
  const { D, G } = gamma(r, th); let mx = 0;
...
console.log('  r      th     compat of harness Gamma      reduction vs explicit');
for (const [r,th] of [[6,1.2],[8,1.2],[12,0.7],[18.2,2.9],[3.8,1.74]]) {
  ...
  console.log('  ' + String(r).padStart(5) + String(th).padStart(7) + '   ' + (compat(r,th)*100).toExponential(2).padStart(12) + '%' + '   ' + (mx*100).toExponential(3).padStart(12) + '%');
}