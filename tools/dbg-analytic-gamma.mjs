...
/* analytic derivatives as the shader's knMetricDR computes them, then Gamma with the correct
   sigma roles: the derivative index runs over {r,th} (stationary + axisymmetric metric), while the
   contracted index rho runs over all four. */
function deriv(r, th) {
...
function gamma(r, th) {
  const D = deriv(r, th); const dg = [D.gr, D.gt, null, null];
  const G = [];
...
function mv(A,v){ return [0,1,2,3].map(i => A[i][0]*v[0]+A[i][1]*v[1]+A[i][2]*v[2]+A[i][3]*v[3]); }
function dot(x,y){ return x[0]*y[0]+x[1]*y[1]+x[2]*y[2]+x[3]*y[3]; }
function reduce(r, th, p, fv) {
  const { D } = gamma(r, th);
  ...
  return [0,1,2,3].map(i => -0.5 * mv(D.gu, q)[i]);
}
function explicit(r, th, p, fv) {
  const { G } = gamma(r, th);
  return [0,1,2,3].map(mu => { let s = 0; for (let al=0;al<4;al++) for (let be=0;be<4;be++) s += G[mu][al][be]*p[al]*fv[be]; return -s; });
}
/* sanity: is the ANALYTIC Gamma compatible? */
function compat(r, th) {
  const { D, G } = gamma(r, th); const dg=[D.gr,D.gt,null,null]; let mx=0;
  for (let sg=0; sg<2; sg++) for (let mu=0;mu<4;mu++) for (let nu=0;nu<4;nu++) {
    const d = dg[sg][mu][nu]; let rhs=0;
...
console.log('  r     th    analytic-Gamma compat     reduction vs analytic-Gamma contraction');
for (const [r,th] of [[6,1.2],[8,1.2],[12,0.7],[18.2,2.9],[3.8,1.74]]) {
  const { D } = gamma(r, th);
  const p = mv(D.gu, [-1, 0.3, 0.2, 3]);
  const fv = [0,0,1/Math.sqrt(D.g[2][2]),0];
  const red = reduce(r, th, p, fv);
  const exp_ = explicit(r, th, p, fv);
  let mx = 0; for (let i=0;i<4;i++) mx = Math.max(mx, Math.abs(red[i]-exp_[i])/Math.max(Math.abs(exp_[i]),1e-30));
  console.log(String(r).padStart(6)+String(th).padStart(7)+'    '+(compat(r,th)*100).toExponential(2).padStart(12)+'%    '+(mx*100).toExponential(3)+'%');
}